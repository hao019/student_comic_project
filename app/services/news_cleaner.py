...
TITLE_SOURCE_SUFFIX_RE = re.compile(
    r"\s*[｜|]\s*(東森新聞|民視新聞|中時新聞網|中央社|聯合新聞網|自由時報|TVBS新聞網|三立新聞網|ETtoday新聞雲|Yahoo奇摩新聞)\s*$"
)
STOP_SECTION_RE = re.compile(
    r"^(廣告\s*/|請繼續往下閱讀|東森新聞\s*\d+\s*頻道|service@|東森電視事業股份有限公司)"
)
PROMO_LINE_RE = re.compile(
    r"(下載東森App|Copyright\s*©|Eastern Broadcasting|豪禮送不停|爆品\$|618年中盛典|線上直播)"
)
